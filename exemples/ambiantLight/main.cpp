...
struct GrassCube : DrawableEntity
{
    ...
    GrassCube(const utils::SharedPtr<gfx::GraphicAPI>& api)
    {
        ...
        graphicPipeline = makePipeline(api, "ambiantLight");
...
)
{
    ...
    api->setFragmentUniform(entt.graphicPipeline->findFragmentUniformIndex("u_ambiantColor"), light.color);
    api->setFragmentUniform(entt.graphicPipeline->findFragmentUniformIndex("u_ambiantIntensity"), light.intensity);
    entt.draw(api);
}

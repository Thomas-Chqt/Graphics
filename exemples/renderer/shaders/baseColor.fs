...
vec3 computePointLight(int lightIdx, vec3 normal, vec3 cameraDir, vec3 baseColor)
{
    ...
    vec3 ambiant  = baseColor           * u_pointLights[lightIdx].color * u_pointLights[lightIdx].ambiantIntensity;
    vec3 diffuse  = baseColor           * u_pointLights[lightIdx].color * u_pointLights[lightIdx].diffuseIntensity  * max(diffuseFactor, 0.0f);
...
void main()
{
    vec4 baseColor = texture(u_diffuseTexture, fragUv)

    vec3 normal = normalize(fragNormal);
    vec3 cameraDir = normalize(u_cameraPos - fragPos);

    fragmentColor = vec4(computePointLight(0, normal, cameraDir, baseColor.xyz), 1.0);
}
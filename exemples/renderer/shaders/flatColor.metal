...
fragment float4 flatColor_fs(
    ...
    constant float3& u_light_direction         [[buffer(2)]],
    ...
    constant float& u_material_ambiant   [[buffer(7)]],
    constant float& u_material_diffuse   [[buffer(8)]],
    constant float& u_material_specular  [[buffer(9)]],
    constant float& u_material_shininess [[buffer(10)]]
)
{
    ...
    float diffuseFactor = dot(normal, -u_light_direction);
    if (diffuseFactor > 0)
        diffuse = u_material_diffuse * u_light_color * u_light_diffuseIntensity * diffuseFactor;

    float specularFactor = dot(normalize(u_cameraPos - in.pos.xyz), normalize(reflect(u_light_direction, normal)));
    if (specularFactor > 0)

...
{
    aiColor3D ambiant, diffuse, specular;
    float shininess;
...
static utils::SharedPtr<gfx::Texture> textureFromFile(const utils::SharedPtr<gfx::GraphicAPI>& api, const utils::String& path)
{
    int width;
    int height;
...
RenderableEntity loadModel(const utils::SharedPtr<gfx::GraphicAPI>& api, const utils::String& filePath)
{
    ...
    RenderableEntity output;
    output.subMeshes = utils::Array<RenderableEntity::SubMesh>(scene->mNumMeshes);
    ...
    for(utils::uint32 meshIndex = 0; meshIndex < scene->mNumMeshes; meshIndex++)
    {
        ...
        output.subMeshes[meshIndex].vertexBuffer = api->newVertexBuffer(vertices);
        output.subMeshes[meshIndex].indexBuffer = api->newIndexBuffer(indices);
        ...
        //     output.subMeshes[meshIndex].pipeline = makePipeline(api, "textured");
        //     output.subMeshes[meshIndex].material = makeMaterial(aiMaterial);
        //     output.subMeshes[meshIndex].diffuseTexture = textureFromFile(api, textureFilePath);
        // }
        // else
        {
            output.subMeshes[meshIndex].pipeline = makePipeline(api, "flatColor");
            output.subMeshes[meshIndex].material = makeMaterial(aiMaterial);
        }
    }

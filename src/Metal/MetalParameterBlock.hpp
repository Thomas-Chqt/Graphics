...
public:
    ...
    MetalParameterBlock(const std::shared_ptr<MetalBuffer>&, size_t offset, const ParameterBlock::Layout&, MetalParameterBlockPool*);
    ...
    inline const std::map<std::shared_ptr<MetalBuffer>, ParameterBlock::Binding>& encodedBuffers() const { return m_encodedBuffers; }
    inline const std::map<std::shared_ptr<MetalTexture>, ParameterBlock::Binding>& encodedTextures() const { return m_encodedTextures; };
    inline const std::map<std::shared_ptr<MetalSampler>, ParameterBlock::Binding>& encodedSamplers() const { return m_encodedSamplers; };
...
private:
    ...
    std::map<std::shared_ptr<MetalBuffer>, ParameterBlock::Binding> m_encodedBuffers;
    std::map<std::shared_ptr<MetalTexture>, ParameterBlock::Binding> m_encodedTextures;
    std::map<std::shared_ptr<MetalSampler>, ParameterBlock::Binding> m_encodedSamplers;

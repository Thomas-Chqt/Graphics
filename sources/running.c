...
/*   Updated: 2023/09/21 19:28:30 by tchoquet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	start_main_loop(void (*func)(void *), void *data)
{
	if (func == NULL)
		return ;
	mlx_loop_hook(global_data()->mlx_ptr, &main_loop_wrapper, (void *[2]){func, data});
...
static void window_clear(void *window)
{
	if (((t_window *)window)->pixels != NULL)
		ft_bzero(((t_window *)window)->pixels, ((t_window *)window)->image_h * ((t_window *)window)->image_w * 4);	
}

static void window_put_image(void *window)
{
	if (((t_window *)window)->mlx_image != NULL)
		mlx_put_image_to_window(global_data()->mlx_ptr, ((t_window *)window)->mlx_win, ((t_window *)window)->mlx_image, 0, 0);
}
